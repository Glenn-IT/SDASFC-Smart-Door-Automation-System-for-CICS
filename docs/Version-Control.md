...
| v3.00 | Rollout restart #2 on top of v2.10 (sidebar/login logo swapped to the real SDASFC logo) — everything unlocked, same as v2.10 | `dashboard.php`, `profile.php`, `users/index.php`, `users/create.php`, `users/edit.php`, `schedules/index.php`, `schedules/user.php`, `reports/index.php`, `reports/export.php`, `api/rfid_scan.php` | — (full system unlocked) |
...
logo). It briefly re-gated Users create/edit, Schedules, Reports, and the RFID API, but that gate
was removed the same day ("Ungated Pages" commit) so v3.00 ends up fully unlocked, same scope as
v2.10, with the real logo carried through.
